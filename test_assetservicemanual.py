...
endpoint = pytest.endpoint_assetservicemanual
user_belonging_service_manual_ids = []
test_created_service_manual_ids = []
...
def test_get_all_with_token():
    global user_belonging_service_manual_ids
    ...
    for record in response_data['data']:
        ...
        user_belonging_service_manual_ids.append(int(record['id']))
...
def test_get_one_belonging_to_user():
    resp = requests.get(endpoint + str(user_belonging_service_manual_ids[-1]), headers=pytest.headers)

    assert (resp.status_code == 200), 'Status code is not 200. Rather found : ' + str(resp.status_code)
    response_data = resp.json()['data'][0]
    assert int(response_data['id']) == user_belonging_service_manual_ids[-1], 'Data not matched! Expected location id: ' \
                                                                              + str(user_belonging_service_manual_ids[-1]) \
                                                                              + ', but found: ' \
...
def test_get_one_not_belonging_to_user():
    resp = requests.get(endpoint + str(user_belonging_service_manual_ids[0] - 1), headers=pytest.headers)
    response_data = resp.json()
    assert (response_data['status'] == 204), 'Status code is not 204. Rather found : ' + str(response_data['status'])
    assert response_data['message'] == 'Solicitud exitosa pero sin resultados', \
        'Data not matched! Expected response message: ' + 'Solicitud exitosa pero sin resultados, but found: ' + \
...
def test_post_create_sending_all_must_have_params():
    global test_created_service_manual_ids
    data = {'name': 'PYTEST asset service manual ' + str(randint(0, 1000)),
            'file_name': 'pytest_asset_service_manual.pdf'}
    ...
    assert response_data['created'] is not None, 'Service manual was not successfully created. \
        Expected a created ID number, but found : ' + str(response_data['created'])
    test_created_service_manual_ids.append(response_data['created'])
...
def test_post_create_sending_all_must_and_might_have_params():
    ...
    data = {'name': 'PYTEST asset service manual ' + str(randint(0, 1000)),
            'file_name': 'pytest_asset_service_manual.pdf',
            'file_url': 'http://myfakeurl.com/servicemanual9203902'}
    resp = requests.post(endpoint, json=data, headers=pytest.headers)
    response_data = resp.json()['data']
    assert (resp.status_code == 200), 'Status code is not 200. Rather found : ' \
                                      + str(resp.status_code)
...
def test_post_create_without_sending_all_must_have_params():
    ...
    data = {'file_name': 'test_asset_service_manual.pdf',
            'file_url': 'http://myfakeurl.com/servicemanual9203902'}
    resp = requests.post(endpoint, json=data, headers=pytest.headers)
    response_data = resp.json()
    assert (response_data['status'] == 204), 'Status code is not 204. Rather found : ' + str(response_data['status'])
...
def test_put_modify_one_belonging_to_user():
    data = {'name': 'PYTEST asset service manual AFTER PUT ' + str(randint(0, 1000))}
    resp = requests.put(endpoint + str(test_created_service_manual_ids[0]), json=data, headers=pytest.headers)
    response_data = resp.json()['data']
    assert (resp.status_code == 200), 'Status code is not 200. Rather found : ' \
                                      + str(resp.status_code)
    assert (response_data['updated'] == 1), 'Service manual could not be modified. \
        Expected updated number: 1, but found : ' + str(response_data['updated'])


def test_put_modify_one_not_belonging_to_user():
    data = {'name': 'PYTEST asset service manual AFTER PUT ' + str(randint(0, 1000))}
    resp = requests.put(endpoint + str(user_belonging_service_manual_ids[0] - 1), json=data, headers=pytest.headers)
    response_data = resp.json()['data']
    assert (response_data['updated'] == 0), 'Service manual was successfully modified, but it was not supposed to. \
        This service manual does not belong to the logged user. Expected updated ID: 0, but found : ' \
...
def test_delete_belonging_to_user():
    if test_created_service_manual_ids:
        for ID in test_created_service_manual_ids:
            try:
                ...
                delete_query = """Delete from is_asset_manual_service where id = """ + str(ID)
                cursor.execute(delete_query)
                connection.commit()


def test_delete_not_belonging_to_user():
    resp = requests.delete(endpoint + str(user_belonging_service_manual_ids[0] - 1), headers=pytest.headers)
    response_data = resp.json()
    assert (response_data['status'] == 204), 'Status code is not 204. Rather found : ' \
                                             + str(response_data['status'])
    assert response_data['message'] == 'Solicitud exitosa pero sin resultados', \
